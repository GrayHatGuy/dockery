...
## Set-up
Clone repo to $HOME ```sudo cd ~/ && git clone https://github.com/GrayHatGuy/dockery.git```

Change to executable and run setup script ```chmod u+x ~/dockery/setup.sh && sudo bash ~/dockery/setup.sh```

_If script fails add scripts to path and make them executable._ ```export PATH="~/dockery/bin/:$PATH" >> ~/.bashrc chmod u+x ~/dockery/bin/*.sh```

⚠️ **Log out for changes to be incorporated**
## Usage
