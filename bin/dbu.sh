...
echo "Backing up..."
docker save $(docker images -q) -o ~/dockbkup/img/dockerimages`date +%d%b%Y`.tar ; sudo iptables-save > ~/dockbkup/img/savedrules`date +%d%b%Y`.txt && echo "Images and routes backed up to ~/dockbkup/"
retVal=$?
if [ $retVal -ne 0 ]; then
echo "Error"
else
